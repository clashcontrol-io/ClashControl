...
- **Stage 2 increment A (in progress) — selection & isolation on merged chunks, NO shaders (proxy/split-out reuse, since shader-LUT can't be runtime-tested here):** `_findMeshByRef` falls back to the off-scene per-element proxy (`element.meshes[0]`) via a new per-model index `_ccProxyElement`/`_ccProxyMeshFor` → selection EdgesGeometry outline works. `ghostOthers` ghosts whole chunks then re-surfaces kept elements as full-material proxy clones (`_keptProxy`), removed in `unghostAll`. Post-process outline (`_ccPostOutlineFor`) guarded to in-scene meshes only (proxies have no parent). Skips instanced elements (`_instanceRef`) to avoid double-render with the split-out path.
- **Stage 2 increment B (TODO):** color-by-class + class/storey hide + large temp-hide on chunks. These bulk-recolor/hide modes need either a per-vertex elementId + DataTexture shader-LUT (the original plan, coexisting with render-style swaps) OR a "rebuild unmerged while in that mode" fallback — decide once increment A is verified. Render styles also don't yet apply to chunks (swap traversal filters on expressId). Only enable chunk-merge by default after B.
