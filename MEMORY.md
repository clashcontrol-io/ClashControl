...
- **Desktop attempt 5 in flight**: Windows MSI failed `Couldn't find a .ico icon` — bundle.icon listed only icon.png; now lists the generated set (32x32/128/128@2x/icon.icns/icon.ico/icon.png). macOS+Linux assets already on the `desktop-v0.1.0` draft release (id 337444538). Verify the Windows job on the post-#614 run.
...
- **CI quirk escalation**: pull_request event delivery died after 167161e — empty commit did NOT retrigger (first time that remedy failed); close/reopen refused ("cannot be reopened"); remedy that worked: close PR, open a NEW PR (#612 → #614). Anonymous GitHub API rate limit (60/h) exhausted by 30 s monitor polls — poll via authenticated MCP instead, or space polls ≥60 s.
